...
- **Profile page** (`/profile`) — a signed-in user can change their own password (verifies the current password, BCrypt-hashes the new one). This is the authenticated self-service path; the logged-out email-link reset is separate and untouched.
