...
public sealed class UpdateVenueEndpointTests : IntegrationTestBase
{
    ...
    [Fact]
    public async Task Patch_AsClubAdmin_UpdatesNameCourtsAndMaxConcurrent_Returns204()
    {
        ...
        var response = await Client.SendAsync(Patch(
            $"/api/clubs/{clubId}/venues/{venueId}", new { name = "Main Court", courts = 4, maxConcurrentMatches = 2 }));

        response.StatusCode.Should().Be(HttpStatusCode.NoContent);
    }

    [Fact]
    public async Task Patch_InvalidMaxConcurrent_Returns400()
    {
        var clubId = await Seeder.CreateClubAsync("Acme", "ACME");
        var venueId = await Seeder.CreateVenueAsync(clubId, "Main Hall", 1);
        ...
        var response = await Client.SendAsync(Patch(
            $"/api/clubs/{clubId}/venues/{venueId}", new { name = "Main Hall", courts = 4, maxConcurrentMatches = 5 }));
    ...
    public async Task Patch_AsNonAdmin_Returns403()
    {
        ...
        var response = await Client.SendAsync(Patch($"/api/clubs/{clubId}/venues/{venueId}", new { name = "X", courts = 2, maxConcurrentMatches = 1 }));
    ...
    public async Task Patch_VenueOfDifferentClub_Returns404()
    {
        ...
        var response = await Client.SendAsync(Patch($"/api/clubs/{clubA}/venues/{venueInB}", new { name = "X", courts = 2, maxConcurrentMatches = 1 }));
