...
@Component({
  ...
  template: `
    <dialog
      ...
      class="m-auto w-full max-w-lg rounded-md border border-slate-300 bg-white p-0 shadow-2xl"
    >
      <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3">
        <h2 class="font-mono text-sm font-semibold uppercase tracking-wider text-slate-900">{{ title() }}</h2>
        <button
          type="button"
          aria-label="Close"
          (click)="dlg.close()"
          class="rounded px-2 py-0.5 font-mono text-slate-500 hover:bg-slate-100"
        >
          ✕
        </button>
      </div>
