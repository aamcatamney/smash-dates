...
@Component({
  ...
  template: `
    <div
      class="mt-2 rounded-md border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950"
    >
      <ul class="grid gap-1">
        @for (m of squad(); track m.playerId) {
          <li class="flex items-center justify-between text-xs">
            <span
              >{{ m.fullName }}
              <span class="text-slate-400 dark:text-slate-500">{{ m.gender }}</span></span
            >
            @if (canManage()) {
              <button
                type="button"
                (click)="remove(m)"
                class="rounded border border-red-300 px-2 py-0.5 text-red-700 hover:bg-red-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-950"
              >
                Remove
              </button>
            }
      ...
      @if (canManage()) {
        <form (ngSubmit)="add()" class="mt-2 flex items-center gap-2">
          <select
            ...
            class="rounded-md border border-slate-300 px-2 py-1 text-xs dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
          >
          ...
          <button
            type="submit"
            [disabled]="!pick.value"
            class="rounded-md bg-slate-900 px-3 py-1 text-xs font-medium text-amber-300 disabled:opacity-50 dark:bg-amber-400 dark:text-slate-900"
          >
            Add
          </button>
        </form>
  ...
  protected add(): void {
    ...
    this.api.addToSquad(this.clubId(), this.teamId(), playerId).subscribe({
      ...
      error: (e: { error?: { title?: string } }) =>
        this.error.set(e?.error?.title ?? 'Could not add player.'),
    });
  }

  protected remove(m: SquadMember): void {
    this.api
      .removeFromSquad(this.clubId(), this.teamId(), m.playerId)
