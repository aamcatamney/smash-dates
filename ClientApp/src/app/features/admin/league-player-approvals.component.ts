    ...
    >
      @for (r of registrations(); track r.id) {
        <li class="flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-3 font-mono text-sm">
          ...
          <span
            [class]="'ml-auto inline-block rounded px-2 py-0.5 text-xs ' + (r.status | statusColor)"
            >{{ r.status }}</span
          >
      ...
      @for (t of transfers(); track t.id) {
        <li class="flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-3 font-mono text-sm">
          ...
          <span class="text-xs text-slate-400 dark:text-slate-500"
            >(release {{ t.releasingApproved ? '✓' : '…' }}, league
            {{ t.leagueApproved ? '✓' : '…' }})</span
          >
          <span
            [class]="'ml-auto inline-block rounded px-2 py-0.5 text-xs ' + (t.status | statusColor)"
            >{{ t.status }}</span
          >
          @if (canManage() && t.status === 'Pending') {
            <button
              type="button"
...
export class LeaguePlayerApprovalsComponent {
  ...
  private refresh(id: string): void {
    this.api.listLeagueRegistrations(id).subscribe({
      next: (r) => {
        this.registrations.set(r);
        this.emitPending();
      },
    });
    this.api.listLeagueTransfers(id).subscribe({
      next: (t) => {
  ...
  protected confirmReg(r: Registration): void {
    ...
    this.api.confirmRegistration(this.leagueId(), r.id).subscribe({
      ...
      error: (e: { error?: { title?: string } }) =>
        this.error.set(e?.error?.title ?? 'Could not confirm.'),
    });
  }

  protected rejectReg(r: Registration): void {
    this.api
      .rejectRegistration(this.leagueId(), r.id)
      .subscribe({ next: () => this.refresh(this.leagueId()) });
  }

  protected approveTransfer(t: Transfer): void {
    this.api
      .leagueApproveTransfer(this.leagueId(), t.id)
      .subscribe({ next: () => this.refresh(this.leagueId()) });
  }

  protected rejectTransfer(t: Transfer): void {
    this.api
      .leagueRejectTransfer(this.leagueId(), t.id)
      .subscribe({ next: () => this.refresh(this.leagueId()) });
  }
}
