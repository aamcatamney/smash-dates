...
- **Known seams to confirm during execution (flagged inline):** the `IntegrationTestBase` connection-factory property name (`Factory` vs other); Dapper positional value-tuple mapping (fall back to named row records if rejected); `StartGame` court id via query vs body (pick one, keep client in step); client role-gating of board controls deferred to API 403 in v1.
